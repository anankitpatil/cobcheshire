        ...
        toolbarTemplate = "<div class='options'> \
          <span class='no-overflow'> \
            <span class='ui-inputs'> \
              ...
              <button class='underline'>sm</button> \
              <!--<button class='header2'>h2</button> \
              <button class='quote'>&rdquo;</button>--> \
			  <button class='addrow'><i class='fa fa-arrow-circle-o-down'></i></button> \
			  <button class='delrow'><i class='fa fa-minus-circle'></i></button> \
            </span> \
          </span> \
        </div>",
      ...
      if (reTag.test(className)) {
        switch(tag) {
		  ...
		  case "addrow":
			...
			$addrow = $(target.parentNode).parents('tr').clone();
			$(target.parentNode).parents('tr').after($addrow);
			return;
			
		  case "delrow":
		    target = window.getSelection().getRangeAt(0).commonAncestorContainer;
			$addrow = $(target.parentNode).parents('tr').remove();
			return;
